...
bool DL_Dxf::processDXFGroup(DL_CreationInterface* creationInterface,
	...
	// Indicates start of new entity or var
		...
		if (groupValue[0]=='$') {
			...
			strncpy_s(settingKey, groupValue, DL_DXF_MAXLINE);
		...
		if (groupCode<DL_DXF_MAXGROUPCODE) {
			...
			if (!handled) {
				...
				strncpy_s(values[groupCode], groupValue, DL_DXF_MAXLINE);
...
int DL_Dxf::getLibVersion(const char* str) {
	...
	if (idx==3) {
		...
		strncpy_s(v[0], str, d[0]);
		v[0][d[0]] = '\0';

		strncpy_s(v[1], &str[d[0]+1], d[1]-d[0]-1);
		v[1][d[1]-d[0]-1] = '\0';

		strncpy_s(v[2], &str[d[1]+1], d[2]-d[1]-1);
		v[2][d[2]-d[1]-1] = '\0';

		strncpy_s(v[3], &str[d[2]+1], d[3]-d[2]-1);
		v[3][d[3]-d[2]-1] = '\0';
